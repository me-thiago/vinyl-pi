...
**Origem:** Relatório de Auditoria 2025-12-03 - Seção 3: Database (7/10)
...
Atualmente `eventType` é uma string livre no schema Prisma:

```prisma
model AudioEvent {
  eventType   String  // ⚠️ Aceita qualquer string
}
```

Isso pode causar inconsistências se diferentes partes do código usarem strings diferentes para o mesmo tipo de evento.
...
- [Relatório de Auditoria](../../audit-report-2025-12-03.md) - Seção 3: Database
...
- [ ] Task 2: Criar script de migração de dados
  - [ ] Mapear strings existentes para enum values
  - [ ] Tratar valores inválidos/desconhecidos

- [ ] Task 3: Atualizar código TypeScript
...
- [ ] Task 4: Atualizar testes
  ...
  - [ ] Adicionar testes para validação de enum

- [ ] Task 5: Aplicar migration em produção
...
model AudioEvent {
  ...
  sessionId   String?
  eventType   EventType  // ← Agora usa enum
...
- SQLite não suporta enums nativamente, Prisma simula com CHECK constraints
- Considerar manter compatibilidade com strings antigas durante transição
