...
**Origem:** Relatório de Auditoria 2025-12-03 - Seção 8: Performance (8/10) + Feedback do usuário

**User Story:**
Como usuário,
quero que a interface carregue rapidamente e que o player continue tocando ao navegar entre páginas,
para ter uma experiência fluida e contínua estilo Spotify.

## Contexto do Problema

O bundle do frontend não está otimizado:

```
# Auditoria identificou:
- Bundle frontend não otimizado para produção (195KB)
- Sem lazy loading de rotas no frontend
- Sem cache headers para assets estáticos
```

Além disso, ao trocar de página o Player é destruído e o áudio para, prejudicando a experiência.

## Critérios de Aceitação

### Code Splitting & Performance
1. [ ] Rotas secundárias implementadas com lazy loading (React.lazy)
...
8. [ ] Player continua tocando ao navegar entre páginas
9. [ ] Página inicial minimalista focada no VinylVisualizer
...
- VinylVisualizer funcional ✅
- useAudioStream hook funcional ✅

## Referências

- [Relatório de Auditoria](../../audit-report-2025-12-03.md) - Seção 8: Performance
...
- [ ] Task 1: Criar Layout compartilhado com PlayerBar
  ...
  - [ ] Mover lógica de áudio (useAudioStream, useStreamingControl) para o Layout
  - [ ] Passar analyser via context ou props para VinylVisualizer

- [ ] Task 2: Refatorar página inicial (minimalista)
  - [ ] Remover cards de features (ALSA, Icecast, Eventos, Histórico)
  - [ ] Remover seção Stack Tecnológico
  - [ ] Remover status cards (Backend/Frontend/Build)
  - [ ] Centralizar VinylVisualizer como protagonista
  - [ ] Adicionar título "Live Vinyl Visualizer" e badge "ON AIR"

- [ ] Task 3: Implementar lazy loading de rotas
  - [ ] Converter imports de páginas secundárias para React.lazy
  - [ ] Manter página inicial (Home) com import estático
...
- [ ] Task 5: Configurar cache headers no backend
  ...
  - [ ] Cache-Control imutável para assets com hash
  - [ ] Cache moderado para outros assets
  - [ ] Integrar no index.ts

- [ ] Task 6: Validar e testar
  - [ ] Verificar player continua tocando entre páginas
...
### Layout Geral (todas as páginas)

```
┌─────────────────────────────────────────────────────────────────┐
│  🎵 Vinyl-OS  v1.0       [Dashboard] [Sessões] [Diag] [Config]  │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│                                                                 │
│                     < Conteúdo da Página >                      │
...
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│  ▶   Backend: ● Ativo      ▁▂▃▅▆▇▅▃▂   🔊━━━━●━━━  52ms   ⚙️   │
└─────────────────────────────────────────────────────────────────┘
     │         │                │            │        │      │
    Play   Status FFmpeg    VU Meter     Volume   Latência  Config
```
...
Backend parado:
┌─────────────────────────────────────────────────────────────────┐
│  ⏸️    Backend: ○ Parado  [Iniciar]         🔊━━━●━━━      ⚙️   │
└─────────────────────────────────────────────────────────────────┘

Backend ativo, player pausado:
┌─────────────────────────────────────────────────────────────────┐
│  ▶️    Backend: ● Ativo                     🔊━━━●━━━   --ms ⚙️  │
└─────────────────────────────────────────────────────────────────┘

Tudo ativo (tocando):
┌─────────────────────────────────────────────────────────────────┐
│  ⏸️    Backend: ● Ativo   ▁▂▃▅▆▇▅▃▂▁       🔊━━━●━━━   52ms ⚙️  │
└─────────────────────────────────────────────────────────────────┘

Erro:
┌─────────────────────────────────────────────────────────────────┐
│  ▶️    Backend: ⚠️ Erro   [Tentar novamente] 🔊━━━●━━━      ⚙️   │
└─────────────────────────────────────────────────────────────────┘
```
...
**ESTRATÉGIA:** Nova UI disponível em `/test` para validação. UI atual em `/` permanece intacta.
...
// Lazy load das páginas na nova UI
...
**Acesso:**
- `http://localhost:5173/` → UI atual (inalterada)
- `http://localhost:5173/test` → Nova UI com Layout + PlayerBar

**Após aprovação:** Trocar rotas e remover UI antiga.

### Home.tsx (página inicial refatorada)
...
export function staticCache(maxAge: number = 86400) {
  return (req: Request, res: Response, next: NextFunction) => {
    // Cache imutável para assets com hash (ex: main.a1b2c3d4.js)
    if (req.path.match(/\.[a-f0-9]{8}\.(js|css|woff2?)$/)) {
      res.setHeader('Cache-Control', 'public, max-age=31536000, immutable');
    }
    // Cache moderado para outros assets estáticos
...
- frontend/src/App.tsx (conteúdo move para Home.tsx, arquivo pode ser removido)
