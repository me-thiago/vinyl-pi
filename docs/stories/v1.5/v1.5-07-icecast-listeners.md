...
**Origem:** Relatório de Auditoria 2025-12-03 - Seção 5: Audio Pipeline (8/10)
...
Atualmente o contador de listeners não está implementado:
...
O Icecast2 fornece um endpoint de estatísticas que pode ser consultado para obter essa informação.
...
- [Relatório de Auditoria](../../audit-report-2025-12-03.md) - Seção 5: Audio Pipeline
...
- [ ] Task 1: Criar serviço de query do Icecast
...
- [ ] Task 2: Integrar com AudioManager
  - [ ] Adicionar método `getListenerCount()` ao AudioManager
...
export async function getIcecastStats(): Promise<IcecastStats | null> {
  ...
  try {
    ...
    return cachedStats; // Retorna cache antigo se disponível
  }
}
...
async getStatus(): Promise<AudioStatus> {
  return {
    ...
    listeners: await getListenerCount(), // ← Implementado
    // ...
  };
}
```
